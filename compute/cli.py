...
from .providers.runpod_rest_schema import RunPodRestSchemaClient
...
def _local_commit(explicit: str | None, allow_dirty: bool) -> str:
    if not allow_dirty:
        ...
        if staged != 0 or tracked != 0:
            raise RuntimeError("Local repository has tracked uncommitted changes. Commit/stash them, or use --allow-dirty-local only when you intentionally want the last commit.")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    gpu_list = gpu_sub.add_parser("list", help="List GPU stock and REST Pod-create compatibility for a data center")
    ...
    gpu_list.add_argument("--rest-compatible-only", action="store_true", help="Hide GPU IDs not accepted by REST POST /pods")
    return parser
...
def _provider(config, disposable: bool):
    if disposable:
        return RunPodDisposableProvider(RunPodDisposableConfig(network_volume_id=config.runpod_network_volume_id, template_id=config.runpod_disposable_template_id, image_name=config.runpod_disposable_image_name, gpu_type_ids=config.runpod_disposable_gpu_type_ids, gpu_count=config.runpod_disposable_gpu_count, name_prefix=config.runpod_disposable_name_prefix, worker_hostname_prefix=config.runpod_disposable_hostname_prefix, container_disk_gb=config.runpod_disposable_container_disk_gb, poll_interval_seconds=config.runpod_poll_interval_seconds), api=_api(config))
    return RunPodProvider(RunPodProviderConfig(pod_id=config.runpod_pod_id, api_base_url=config.runpod_api_base_url, worker_hostname=config.worker_hostname, request_timeout_seconds=config.runpod_request_timeout_seconds, poll_interval_seconds=config.runpod_poll_interval_seconds, capacity_retry_timeout_seconds=config.runpod_capacity_retry_timeout_seconds, capacity_retry_interval_seconds=config.runpod_capacity_retry_interval_seconds))
...
def _run_preflight(args: argparse.Namespace) -> int:
    ...
    if args.disposable_worker:
        accepted = RunPodRestSchemaClient(request_timeout_seconds=config.runpod_request_timeout_seconds).pod_create_gpu_type_ids()
        invalid = [gpu for gpu in config.runpod_disposable_gpu_type_ids if gpu not in accepted]
        if invalid:
            raise RuntimeError("Configured disposable GPU ID(s) are not accepted by RunPod REST POST /pods: " + ", ".join(invalid) + ". Run `python -m compute.cli gpu list --data-center <DC>` and choose a row with REST=YES. MIG IDs may appear in GraphQL availability while remaining unavailable to REST Pod creation.")
    ...
    request = PreflightRequest(commit_sha=commit, repo_path=config.worker_repo_path, git_remote_url=config.git_remote_url, provider_ready_timeout_seconds=int(config.provider_ready_timeout_seconds), transport_ready_timeout_seconds=int(config.transport_ready_timeout_seconds), transport_poll_interval_seconds=config.transport_poll_interval_seconds, keep_worker_on_failure=args.keep_worker_on_failure, start_worker=not args.existing_worker)
...
def _run_gpu(args: argparse.Namespace) -> int:
    ...
    accepted = RunPodRestSchemaClient(request_timeout_seconds=config.runpod_request_timeout_seconds).pod_create_gpu_type_ids()
    for gpu in gpus:
        gpu["restCompatible"] = gpu.get("id") in accepted
    if args.rest_compatible_only:
        gpus = [gpu for gpu in gpus if gpu["restCompatible"]]
    ...
    if not gpus:
        ...
        return 0
    print(f"{'STOCK':<10} {'VRAM':>6} {'PRICE/HR':>10} {'REST':>5}  {'GPU ID'}")
    for gpu in gpus:
        ...
        rest = "YES" if gpu.get("restCompatible") else "NO"
        print(f"{str(gpu.get('stockStatus', '?')):<10} {memory:>6} {price:>10} {rest:>5}  {gpu.get('id', '')}")
    print("REST=YES means the current RunPod OpenAPI schema accepts this exact ID in POST /pods gpuTypeIds.")
    print("MIG availability may be visible through GraphQL even when REST Pod creation does not accept that MIG ID.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "preflight": return _run_preflight(args)
        if args.command == "volume": return _run_volume(args)
        if args.command == "gpu": return _run_gpu(args)
        parser.error(f"Unsupported command: {args.command}")
    except KeyboardInterrupt:
        print("Interrupted", file=sys.stderr); return 130
    except Exception as exc:
        print(f"KRITI COMPUTE FAILED: {exc}", file=sys.stderr); return 1
    return 2
